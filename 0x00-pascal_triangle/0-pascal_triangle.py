...
def pascal_triangle(n):
    ...
    for i in range(n):
        ...
        for idx, item in enumerate(psl_trg[i]):
            if len(psl_trg[i]) == 1:
                # if we have only 1 element in the main list, we need not do anything in this loop
                break
            if idx == 0:
                # We don't want to touch the first element of the inner list which will always be 1
                continue
